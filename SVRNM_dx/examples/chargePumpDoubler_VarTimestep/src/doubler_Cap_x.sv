...
module doubler_Cap_x import EE_pkg::*; (
  ...
  parameter real Tmin=1e-9;   // Min sample rate 
  parameter real Tmax=10e-6;  // Max sample rate
  ...
  CapDx #(.c(Cp), .tmin(Tmin), .tmax(Tmax), .reltol(0.005), .vtol(0.001)) Cpump (MID,VCK);
  CapGx #(.c(CL), .tmin(Tmin), .tmax(Tmax), .reltol(0.005), .vtol(0.001)) Cout  (OUT);
