...
//Verilog HDL "clockstim" "functional"
//This block is used to create a clock signal with parameterized frequency

`timescale 1ns/1fs

module Clockstim import cds_rnm_pkg::*; (clk);

output wreal4state clk;
reg clk_reg;

parameter real freq = 1e3;      //Frequency of the sampling clock signal in Hertz(Hz) 
parameter logic ic = 0;         //Initial value of sampling clock
parameter real ampl = 2.0;      // Output amplitude

real pulse_width = (0.5*1s)/freq;    

initial begin
	clk_reg = ic;                          //Clock initialization
end

always begin
	#pulse_width clk_reg = !clk_reg;           //Clock generation
end

assign clk = clk_reg*ampl;
