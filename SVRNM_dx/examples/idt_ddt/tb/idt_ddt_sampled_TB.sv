...
real IN, IDT, DDT, INsamp, DDTsamp, IDTsamp;
...
samp  #(.Ts(20ps))  Isamp(.IN(IN), .OUT(INsamp));	// added sampler

idt_real  Iidt2 (.IN(INsamp),.OUT(IDTsamp)); // derivative of sampled input

ddt_real  Iddt2 (.IN(INsamp),.OUT(DDTsamp)); // integral of sampled input
