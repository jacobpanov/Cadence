...
import cktImpairments_pkg::*;
logic active = 1;
c_pgaImps pgaImps = new;
...
initial begin                        // MAIN TEST PROCEDURE
  result = setPGARand();             // Call randomization function
...
function logic setPGARand ();
     logic randOK;
     if (active == 1'b1)
        randOK = pgaImps.randomize();
     if (randOK) begin
        $display("Randomized PGA Imps.");
        $display("   gainErr = %f dB", pgaImps.gainError);
        $display("   centerOffset = %f V", pgaImps.centerOffset);
        pga_TB.pga.gainErr = pgaImps.gainError;
        pga_TB.pga.centerOffset = pgaImps.centerOffset;
     end
     return randOK;
endfunction
