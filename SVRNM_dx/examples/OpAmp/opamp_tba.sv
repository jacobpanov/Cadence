...
// --------------------------------------------------------------
// Test Bench for Op Amp (ELECTRICAL FORMAT)
//                       
//                 (N)             (OUT)
//      Vin ---RI---+-------RF-------+
//                  |  |'-._         |
...
EVIRsrcG Vd (VDD, Vdd, 0.0, 0.0);         // drive supply nets
EVIRsrcG Vs (VSS, Vss, 0.0, 0.0);

opamp1 #(.sr(sr),.ugf(ugf),.av(av))      // op amp with all electrical nets
          Op (OUT, P,N, VDD,VSS);

EVIRsrcG #(.tr(tinc))  Vm (N, Vin,  0.0, RI );
EVIRsrcG #(.tr(tinc))  Vp (P, Vref, 0.0, 0.0);
EVIRsrcG #(.tr(tinc))  Rl (OUT,    0.0, 0.0, RL );
EVRsrcD #(.tr(tinc))   Rf (N,OUT,  0.0, RF,  Irf);
EVRsrcD #(.tr(tinc))   Rs (OUT,CO, 0.0, RSW, Icl);
ECapG    #(.c(c))      Cl (CO);

real VOUT;                   // measured output voltage
EVmeas  Me (OUT, CK, VOUT);  // samples at all input timesteps
...
//Magnitude measurement (output center assumed at 2.0 volts):
real Vhi,Vlo,Mag,Mnom;
always @(Freq) begin              // start when freq changes
  Vhi=2; Vlo=2;                   // set max&min to center
  while (VOUT<=2) @(VOUT) #1;     // skip past low halfcycle
  while (VOUT>=2) @(VOUT) #1;     // skip past high halfcycle
  while (VOUT<=2) @(VOUT) #1;     // skip past low halfcycle
  while (VOUT>=2) begin           // For high halfcycle:
    if (VOUT>Vhi) Vhi=VOUT;       // save max value
    @(VOUT) #1;                   // and repeat
  end
  while (VOUT<=2) begin           // For low halfcycle:
    if (VOUT<Vlo) Vlo=VOUT;       // save min value
    @(VOUT) #1;                   // and repeat
  end
  Mnom = 10/$sqrt(1+(Freq/1e6)**2); // compute expected magnitude
  Mag = (Vhi-Vlo)/2/Vpk;          // compute magnitude wrt Vpk

/*  Vhi=2; Vlo=2;                   // set max&min to zero
  repeat(2) begin                 // Skip first 1 to 2 cycles:
    while (VOUT>=2) @(VOUT) #1;   // skip past high input
    while (VOUT<=2) @(VOUT) #1;   // skip past low input
  end
  while (VOUT>=2) begin           // For high halfcycle:
    if (VOUT>Vhi) Vhi=VOUT;       // save max value
    @(VOUT) #1;                   // and repeat
