...
// --------------------------------------------------------------
//
// NMOS_EE (D,G,S) - EEnet fully diffl NMOS switch
// Parameter vth defines minimum Vgs or Vgd for conduction.
...
parameter real vth=0.6;       // gate-source voltage required for turnon
...
real Vgs,Vgd;                 // controlling voltages
reg en;                       // enable state (high or low)
real Ids;                     // drain-source current

assign Vgs = G.V-S.V;         // variable for gate-source voltage
assign Vgd = G.V-D.V;         // variable for gate-drain voltage

initial forever begin
  if (Vgs>vth || Vgd>vth) en=1'b1; // on when either junction above threshold
