...
parameter real vTol = 1e-15; // Tolerance for convergence

real T0,IN0,V0;      // values at previous timestep
real Tn,INn,Vout;    // values on most recent iteration
int Niter;           // iteration counter
real dvOld, dvNew;   // value to hold the delta-V computed last iteration
                     // and the new delta-V
 ...
 if (IN<1e20) begin             // (ignore all X, Z, NaN points)
   if ($realtime==0) INn=IN;    // at T=0, just update IN value
     ...
     if ($realtime>Tn) begin    // if new forward step
       ...
       dvOld=0;                 // Initialize dV tracker
     end 
     else Niter++;              // increment iteration counter
     if (Niter<=maxiter) begin  // limit iterations at a timepoint
       dvNew = (K/2)*(IN+IN0)*($realtime-T0);
       if (myAbs(dvOld - dvNew) > vTol) // check against tolerance
          Vout = V0 + dvNew;  // integrate with new value
       else
          Vout = V0 + dvOld;  // Use old value, i.e. don't update Vout
       dvOld = dvNew;
     end
     else $display("WARNING: Maximum number of iterations exceeded at time %t",$realtime);
