...
module ADC_8bit import cds_rnm_pkg::*; import EE_pkg::*; ( 
   Vinp, Vinm, clkp, clkn, fclkp, fclkn, ADCEn, outi, outq, 
   outClkI, outClkQ, VDD, VSS, VREF1, VREF2, VCM );
...
//  Current source to model active core (background) current
 ...
 assign supplyOK = vddGoodFilt & vssGoodFilt;
  ...
  assign outi = (adcOn) ? latch_reg[1] : 8'bz;
  assign #2 outClkI = ~clk_div2 & adcOn;
  assign outq = (adcOn) ? latch_reg[0] : 8'bz; 
