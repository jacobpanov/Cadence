...
always @(Vclk) begin		// on leading clock edge
  if (tupd>0) fdig=0.5*1s/($realtime-tupd);	//  compute F=1/(period*2) (Hz)
  tupd = $realtime;                     //   and save edge time
end

// MEASURE ACTUAL SINUSOID FREQUENCY:
real fsin,tups=0;  		// measure actual sinusoid frequency:
real VsinTemp, lastVsin;        // temporary holders for Vsin value
real tupInt, lastTime, thisTime; // interpolated time of zero-crossing
always @ (Vsin) begin
   lastVsin = VsinTemp;
   lastTime = thisTime;
   VsinTemp = Vsin;
...
assign Dsin = (Vsin>0);		// digital signal: high when sine>0
always @ (Dsin) begin		// on any transition
  tupInt = $realtime - (myAbs(Vsin) * ($realtime - lastTime) / (myAbs(Vsin) + myAbs(lastVsin)));
  if (tupInt>0) fsin=0.5*1s/(tupInt-tups);	//  compute F=1/(period*2) (Hz)
  tups = tupInt;			//   and save edge time
end

function real myAbs (real In);
   return (In < 0) ? -In : In;
endfunction
