...
module adc_dac_TB ( );
  ...
  initial begin 					// real input controls
    AIN=-3.05; REFH=3; REFL=0;				// initial input settings
    while (AIN < 3.05)  #20 AIN = AIN + 3.0/255;	// slow input ramp up
    while (AIN > -3.05) #20 AIN = AIN - 0.06;	 	// faster ramp down
  ...
  initial begin 					// clock generator
    ...
    repeat (1100) #10 CK=!CK;				// clock @ 20ns
    ...
    repeat (300) #10 CK=!CK; 				// OK clock
  end

  adc #(.Nbits(8)) adc8(DOUT8, AIN, CK,REFH,REFL);	// 8-bit ADC
  adc #(.Nbits(4)) adc4(DOUT4, AIN, CK,REFH,REFL);	// 4-bit ADC
