...
SUMMARY OF USEFUL INFO:
 Tau = sqrt(C*L) = 10ns
 Period = 2*pi*Tau = 63ns
 Circuit provides SV simulation using IndDeq0 and CapGeq0, as well as a 
 VerilogAMS simulation of the electrical circuit.

 Timestep size chosen bounds the transient accuracy.  This variation can be 
 seen by adjusting the Ts parameter below and comparing the measured period 
 of the oscillatory response (which is where the transient model's effects 
 dominate the results).  Static DC levels are only limited by net resolution
 evaluation controls (EEIO's vtol and itol limit voltage and current accuracy).

 Note that electrical simulation results used for comparison must use 
     errpreset=conservative  method=trapezoidal
 in order to see optimal accuracy in the lightly-damped analog response.
 Default of errpreset=moderate typically provides about 1% transient accuracy.
 Using method=gear for analog simulation would not accurately model the lightly
 damped response (trapezoidal integration is required for high-Q systems, as
 gear algorithms insert non-trivial excess damping).

 Typical relation between sample rate to time constant ratio and the
 resulting time tolerance is approximately:
    ttol = 0.08*(Ts/Tau)^2
**********************************************************/
...
//+++ Added parameter for sample period here, then used in the L&C models:
...
//+++ Added capacitor and inductor models: 
IndDeq0 #(.l(L), .ic(0), .tinc(Ts))  L1 (A,B);   // EEnet inductor model
CapGeq0 #(.c(C), .ic(0), .tinc(Ts))  C1 (B);     // EEnet capacitor model

//+++ Or to use the more-complex models at op point, use this instead:
// IndD   #(.l(L), .tinc(Ts))  L1 (A,B);   // EEnet inductor model
// CapGeq #(.c(C), .tinc(Ts))  C1 (B);     // EEnet capacitor model
