...
module CapDeq0(inout EEnet P,N);
...
parameter real tinc=1e-9;  // timestep for computing update (sec)
...
VRsrcD VC(.P, .N, .vval(CD.Veq), .rval(CD.Req));

// Drive current into capacitor model:
assign Icap = VC.DP.Imeas; // current to plot or measure with OOMR 
assign DV = '{0,VC.DP.Imeas,`wrealZState};

// Capacitor model converts from input current to output voltage:
CapGeq0 #(.c(c), .rs(rs), .ic(ic), .tinc(tinc)) CD(DV);
