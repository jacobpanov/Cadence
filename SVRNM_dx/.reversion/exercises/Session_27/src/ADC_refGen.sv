...
//   trim1 varies the center point of the 3 outputs, which
//     is the same as VCM. All 3 outputs move up and down with 
//     this control.
//   trim2 varies the gap between the upper and lower references
//     VREF1 and VREF2, and the center point VCM. This provides
//     a way to control the LSB size and optimize the ADCs dynamic range.
//
//--------------------------------------------------------------------------------------

`timescale 1ns / 1ps

module ADC_refGen import cds_rnm_pkg::*; import EE_pkg::*; (
   vdd, vss, VCM, VREF1, VREF2, trim1, trim2, en );
...
//  Current source to model active core (background) current
  ...
  always @ (en or Active)
     if ((en === 1'b1) && (Active === 1'b1)) adcEn = 1'b1;
     else adcEn = 1'b0;
  ...
  always @ (adcEn)
     begin
        if (adcEn == 1'b1) begin
           #(turnOn * 1s);
           if (adcEn == 1'b1) // make sure it is still enabled before turning on
              adcOn = 1'b1;
           else
              adcOn = 1'b0;
        end
  ...
  always @ (trim2_int or trim1_int or vdd.V or vss.V or adcOn or Active) begin
     if (adcOn == 1'b1) begin
        #(5);
        vcm_int = (trim1_int * trim1Step) + (vdd.V - vss.V)/2;
        vref1_int = vcm_int + vrefNom + (trim2_int * trim2Step); // vcm + offset
        vref2_int = vcm_int - vrefNom - (trim2_int * trim2Step); // vcm - offset
     end
     else begin
