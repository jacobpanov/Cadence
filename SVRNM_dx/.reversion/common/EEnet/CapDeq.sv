...
parameter integer rxflag=1; //   0=turn off bigstep feature,
                            ...
                            //   2=assume OOMR used to add Rx value to CD.RxExt
...
CapGeq #(.c(c), .rs(rs), .ic(ic), .tinc(tinc), .rxflag(rxflag>0?2:0))  CD(DV);
...
initial if (ic<1e9) Rd = 1e12;  // with IC, cap works same at DC as transient.
  ...
  while ($realtime==0) begin        // update on each voltage change
    if (P.V-N.V<1e9) Vd = P.V-N.V;  // pass DC input voltage to cap
    @(P.V,N.V,Icap);                // repeat at each DC input voltage change
  end
  Vd=0; Rd = 1e12;                  // then go to current-only drive when T>0
end

initial if (rxflag==1)              // get external R for cap BigStep mode:
