...
module dualBBA( inim, inip, inqm, inqp, vdd, vss, en, outim, outip, outqm, outqp, bwTrim, gain, dcocip, dcocim, dcocqp, dcocqm
);
...
//  Current source to model active core (background) current
  ...
  always @ (bwTrim or enInt)
     begin
        if ( (^bwTrim !== 1'bx) && (^bwTrim !== 1'bz) && (enInt == 1'b1) )
  ...
  always @ (gain or enInt)
     begin
        if ( (^gain !== 1'bx) && (^gain !== 1'bz) && (enInt == 1'b1) ) begin
