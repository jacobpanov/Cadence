...
// randomly update a register
 task automatic regchange(ref logic [7:0] volatilereg, input string id);
   int ok, cycles;
   repeat(5) begin
     // random cycles between 2 and 5
     ok = randomize(cycles) with {cycles > 1; cycles < 6;};
     $display(" DUT:wait %0d cycles to update %0s", cycles, id);
     repeat(cycles) @(posedge clk);
     volatilereg = cycles;
     $display("DUT: %0t %s updated %0d",$time, id, cycles);
   end
 endtask

  initial begin
    fork
      regchange(reg_six,"reg_six");
      regchange(reg_seven,"reg_seven");
    join
  end
