...
// User register sequence
	...
	class qt_test extends uvm_test;
		custom_regs_vendor_Cadence_Design_Systems_library_Custom_Registers_version_1_5 model;
		UVC#(string) hi_uvc0 = new("HI0",null);

		virtual function void build();
			reg2simple_adapter adapter = new();
			uvm_reg::include_coverage("*", UVM_CVR_ALL);
			model=custom_regs_vendor_Cadence_Design_Systems_library_Custom_Registers_version_1_5::type_id::create("model", this);
			model.build();
			model.set_hdl_path_root("custom_regs");
			model.custom.set_auto_predict(1);
			model.custom.set_sequencer(hi_uvc0.sqr,adapter);
			model.lock_model();
